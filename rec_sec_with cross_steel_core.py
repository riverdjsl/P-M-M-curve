...
bf = number_input("input the b-flange width:")
hs = number_input("input the inner height:")
hf = number_input("input the h-flange height:")
...
f = open("rec_sec_with_cross_core.txt", "w")
f.write("30,")
f.write("\n")
...
for i in [k*els_s_b+xs0+els_s_b/2+(bs-bf)/2 for k in range(int(eles_num))]:
	for j in [ys0, h-ys0]:
		f.write('3,{:.1f},{:.1f},{:.1f},'.format(els_s_b*tw, i, j))
		f.write("\n")

for i in [k*els_s_h+ys0+els_s_h/2+(hs-hf)/2 for k in range(int(eles_num))]:
	for j in [xs0, b-xs0]:
